import type { BibleAccount } from "./types";

const jehoshaphat: BibleAccount = {
  id: "jehoshaphat",
  name: "Jehoshaphat",
  title: "\"We don't know what to do, but our eyes are on you\"",
  emotionTags: ["fearful", "anxious"],
  lifeSituations: [
    "overwhelming-odds",
    "seeking-god",
    "national-crisis",
    "prayer",
  ],
  overview:
    "King Jehoshaphat learned that a vast army was advancing against Judah. Scripture says he \"resolved to inquire of the Lord\" and proclaimed a fast for the whole nation. In his prayer, he admitted plainly, \"We do not know what to do, but our eyes are on you.\" God responded through a prophet, telling the people not to be afraid because the battle was not theirs but God's. The next day, Jehoshaphat appointed singers to go ahead of the army praising God, and the opposing armies were routed before Judah had to fight.",
  whyRelevant:
    "Jehoshaphat faced a threat large enough that he openly admitted he didn't know what to do. His account speaks to fear that comes from a real, specific threat, and shows a response of prayer and honesty rather than pretending to have it together.",
  fullBibleReference: "2 Chronicles 20:1-30",
  keyScriptures: [
    {
      reference: "2 Chronicles 20:3",
      text: "Alarmed, Jehoshaphat resolved to inquire of the Lord, and he proclaimed a fast for all Judah.",
    },
    {
      reference: "2 Chronicles 20:12",
      text: "We have no power to face this vast army... We do not know what to do, but our eyes are on you.",
    },
    {
      reference: "2 Chronicles 20:15",
      text: "Do not be afraid or discouraged because of this vast army. For the battle is not yours, but God's.",
    },
  ],
  reflectionQuestions: [
    "Jehoshaphat admitted out loud that he didn't know what to do. What would it look like for you to be that honest about your fear?",
    "He gathered the whole nation to seek God together rather than facing the threat alone. Who could you bring into this fear with you?",
    "Jehoshaphat was told the battle wasn't his to fight alone. What are you carrying right now that might not be entirely yours to carry?",
  ],
  prayerPrompt:
    "God, like Jehoshaphat, I don't know what to do, but my eyes are on you. Help me bring my fear honestly instead of pretending I have this figured out.",
  estimatedReadingTime: 5,
  searchKeywords: [
    "overwhelmed",
    "dont know what to do",
    "don't know what to do",
    "outnumbered",
    "facing something big",
    "scared",
    "afraid",
  ],
  scenes: [
    {
      id: "a-vast-army-approaches",
      title: "A vast army approaches.",
      narrative:
        "Jehoshaphat received word that a huge coalition of armies was advancing against Judah.",
      scriptureReferences: [
        { reference: "2 Chronicles 20:2", text: "A vast army is coming against you from Edom, from the other side of the Dead Sea." },
      ],
    },
    {
      id: "alarmed-he-sought-the-lord",
      title: "Alarmed, he sought the Lord.",
      narrative:
        "Afraid, Jehoshaphat proclaimed a fast for the whole nation and prayed publicly, admitting plainly, \"We do not know what to do, but our eyes are on you.\"",
      scriptureReferences: [
        { reference: "2 Chronicles 20:12", text: "We have no power to face this vast army... We do not know what to do, but our eyes are on you." },
      ],
    },
    {
      id: "the-battle-is-not-yours",
      title: "\"The battle is not yours.\"",
      narrative:
        "A prophet, Jahaziel, told the assembly not to be afraid, because the battle belonged to God, not to them, and they would not even need to fight.",
      scriptureReferences: [
        { reference: "2 Chronicles 20:15", text: "Do not be afraid or discouraged because of this vast army. For the battle is not yours, but God's." },
      ],
    },
    {
      id: "singers-before-the-army",
      title: "Singers before the army.",
      narrative:
        "The next morning, Jehoshaphat appointed singers to go out ahead of the army praising God. As they sang, the opposing armies turned on each other and were defeated.",
      scriptureReferences: [
        { reference: "2 Chronicles 20:22", text: "As they began to sing and praise, the Lord set ambushes against the men... who were invading Judah, and they were defeated." },
      ],
    },
  ],
};

export default jehoshaphat;
